...
import com.example.clothloop.data.model.User
...
class logInViewModel(context: Context) : ViewModel() {
    ...
    suspend fun editData(email: String, name: String, birthDate: String, phoneNumber: String, address: String) {
        val doc = db.collection("user").whereEqualTo("email", email)
        val snapshot = doc.get().await()

        if (!snapshot.isEmpty) {
            for (document in snapshot.documents) {
                db.collection("user").document(document.id).update(mapOf(
                    "name" to name,
